...
**Status:** **Draft — awaiting owner acceptance.** Merging the PR that carries this document is the owner acceptance act. Every item marked **OWNER DECISION** below carries a proposed default; the owner accepts the defaults by merging unedited, or edits them before merge. **This decision authorizes no code change by itself**, and even once accepted it authorizes no implementation PR until the owner flips the corresponding §7 row. This is **not** production scoring law: everything below is **testnet-provisional** unless separately re-governed.
**Date:** 2026-07-12 (draft authored; the acceptance date is the merge date of the PR carrying this document)
...
|---|---|---|---|
| **PR-UWR-RUNTIME-LOADER** | afi-core | Pure `loadUwrProfile(profileJson)` validator mapping a registry document onto `UniversalWeightingRuleConfig` per the RC-5 predicate; no `fs`; no afi-config dependency; subpath export; unit tests; may update the `defaultUwrConfig` TODO comment to reference this decision (RC-8) | No — owner flips this row |
