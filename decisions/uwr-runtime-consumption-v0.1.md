...
| **PR-UWR-RUNTIME-READ** | afi-reactor | Composition-root registry read behind the RC-3 flag (default `builtin`); RC-4 fail-closed semantics; cwd-independent path resolution preferred; guardrail amendment exactly per RC-7 grant (1); new tests jest-allowlisted | **Yes — row flip merged by owner per RC-12 (recorded 2026-07-13)** |
| **PR-UWR-STAMP-SEMANTICS** | afi-reactor | RC-6 source discriminator + consumed-profile stamp condition; guardrail stamp-shape/wiring assertions updated exactly per RC-7 grant (2) | **Yes — row flip merged by owner per RC-12 (recorded 2026-07-13)** |
