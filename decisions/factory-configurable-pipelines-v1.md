...
- **`uwr-runtime-consumption-v0.1.md`** — unchanged; its ledger and RC-3 flag default are not advanced (RC-3 flip expressly excluded, §12); its RC-6 stamp-semantics meaning is preserved while the froggy-only stamp gate is replaced by registry-backed stamping (D-FCP-9 item 5).
